...
{
    CGRect frame = (CGRect){ {100.0, 100.0}, {512.0, 512.0} }; // window size : width and height 
    ...
    _pDevice = MTL::CreateSystemDefaultDevice(); // set device init 

    _pMtkView = MTK::View::alloc()->init( frame, _pDevice ); // frame 
    _pMtkView->setColorPixelFormat( MTL::PixelFormat::PixelFormatBGRA8Unorm_sRGB ); // RGB bit setting 
    _pMtkView->setClearColor( MTL::ClearColor::Make( 1.0, 0.0, 0.0, 1.0 ) ); // clear color -> glClearColor  

    _pViewDelegate = new MyMTKViewDelegate( _pDevice ); 
    _pMtkView->setDelegate( _pViewDelegate );

    _pWindow->setContentView( _pMtkView );
    _pWindow->setTitle( NS::String::string( "Hello MetalAPI", NS::StringEncoding::UTF8StringEncoding ) );
